        ...
        <div class="max-w-7xl mx-auto py-3 px-3 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between flex-wrap">
            ...
            <div class="order-3 mt-2 shrink-0 w-full sm:order-2 sm:mt-0 sm:w-auto">
              <button @click.prevent="showModal = true" class="flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-xs text-sm font-medium text-yellow-600 bg-white hover:bg-yellow-50">
                {{ __('No, change it') }}
              </button>
            </div>
            <div class="order-2 shrink-0 sm:order-3 sm:ml-3">
              <button @click.prevent="manualShow = false" type="button" class="-mr-1 flex p-2 rounded-md hover:bg-yellow-500 focus:outline-hidden focus:ring-2 focus:ring-white sm:-mr-2">
