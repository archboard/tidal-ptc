  ...
  <RadioGroup v-model="localValue">
    <div class="-space-y-px rounded-md bg-white dark:bg-gray-900">
      <RadioGroupOption as="template" v-for="(option, optionIndex) in options" :key="option.label" :value="option.value" v-slot="{ checked, active }">
        <div :class="[optionIndex === 0 ? 'rounded-tl-md rounded-tr-md' : '', optionIndex === options.length - 1 ? 'rounded-bl-md rounded-br-md' : '', checked ? 'z-10 border-primary-200 dark:border-primary-700 bg-primary-50 dark:bg-primary-900' : 'border-gray-200 dark:border-gray-700', 'relative flex cursor-pointer border p-4 focus:outline-hidden']">
