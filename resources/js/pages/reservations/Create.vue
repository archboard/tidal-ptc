<template>
  <Authenticated>
    <CardWrapper>
      <CardPadding>
        <CardHeader>{{ student.name }} · {{ staff.name }}</CardHeader>
      ...
      <CardPadding v-if="slots.length === 0">
        {{ __('There are no available times right now.') }}
      </CardPadding>

      <CardPadding v-for="(daySlots, day) in slotsByDay" :key="day">
      ...
      <CardPadding v-if="selected">
        <form @submit.prevent="submit" class="space-y-4">
          <p class="font-medium">{{ displayDate(selected.starts_at, 'full') }}</p>
...
const slotsByDay = computed(() => props.slots.reduce((carry, slot) => {
...
const submit = () => {
  if (props.existingReservation) {
    form.transform(data => ({ ...data, time_slot_id: selected.value.id }))
      .put(`/reservations/${props.existingReservation.id}`)
  } else {
    form.post(`/reservations/${selected.value.id}`)
  }
}
</script>
